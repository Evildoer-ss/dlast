...
def get_parsed_args():
    ...
    parser.add_argument("-i", "--input", type=str, help='Input cg.json generated by iblessing')
    parser.add_argument("-p", "--plist", required=True, type=str, help="Input APP's Info.plist")
    parser.add_argument('-o', '--output', type=str, help='Not useful at the moment')
    parser.add_argument("-d", "--debug", action="store_true", help='DEBUG')
    parser.add_argument("binary", type=str, help="ipa's executable file")

    args = parser.parse_args()
    if not os.path.exists(args.binary) or not os.path.isfile(args.binary):
        print('input file???')
        sys.exit(1)
    if not os.path.exists(args.plist) or not os.path.isfile(args.plist):
        print('Info.plist???')
        sys.exit(1)
...
def main():
    ...
    # get url scheme
    plist = readPlist(args.plist)
    URLSchemes = []
    if 'CFBundleIdentifier' not in plist.keys():
        print('Not found CFBundleIdentifier in Info.plist')
    if 'CFBundleURLTypes' not in plist.keys():
        print('Not found CFBundleURLTypes in Info.plist')
    for item in plist['CFBundleURLTypes']:
        URLSchemes += item['CFBundleURLSchemes']
    open(os.path.join(TMP_DIR, 'URLSchemes.json'), 'w').write(json.dumps(URLSchemes))

    generate_fixed_json_file(args.binary)

    print('Generateing corpus...')
    helper_lib = ctypes.CDLL(os.path.join(PROJ_ROOT_PATH, 'libs', 'libhelper.dylib'))
    helper_lib.GenerateCorpus(TMP_DIR.encode())

    # start fuzz
    fuzz(TMP_DIR, plist['CFBundleIdentifier'])
